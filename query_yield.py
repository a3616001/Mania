...
def query_Id_Id_big(id1, id2, json1, json2):
	...
	if json1.has_key('F') and json2.has_key('F'):
		for FId in join(FIdList1, FIdList2):
	...
	if json1.has_key('AA') and json2.has_key('AA'):
		for AuId in join(AuIdList1, AuIdList2):
	...
	if json1.has_key('RId'):
		...
		RIdList.sort()
		Id2citedList.sort()
		for RId in join(RIdList, Id2citedList):
	...
	if json1.has_key('RId'):
		...
		if json2.has_key('F'):
			for id1CitePaper in id1CitePapersInfo:
				if id1CitePaper.has_key('F'):
					...
					FIdListTmp.sort()
					for FId in join(FIdListTmp, FIdList2):
		...
		if json2.has_key('AA'):
			for id1CitePaper in id1CitePapersInfo:
				if id1CitePaper.has_key('F'):
					...
					AuIdListTmp.sort()
					for AuId in join(AuIdListTmp, AuIdList2):
		...
		for id1CitePaper in id1CitePapersInfo:
			if id1CitePaper.has_key('RId'):
				...
				RIdListTmp.sort()
				for RId in join(RIdListTmp, Id2citedList):
					...
					if paper.has_key('AA'):
						for author2 in paper['AA']:
							if author2['AuId'] == auId2 and author2.has_key('AfId') and (author2['AfId'] in AFIdSet1) and not((author2['AfId'], auId2) in uniqueSet):
								answer(ans, [auId1, author2['AfId'], auId2, id2])
...
def query_AuId_AuId(auId1, auId2, json1, json2):
	...
	if len(AFIdSet1)>0 and len(AFIdSet2)>0:
		...
		for AFId in AFIdSet1.intersection(AFIdSet2):
			answer(ans, [auId1, AFId, auId2])
	print len(ans)
	#AuId-Id-AuId
	if len(IdList1)>0 and len(IdList2)>0:
		for id in join(IdList1, IdList2):
...
def main():
	...
	now = time.time()
	print len(query(621499171, 2100837269))
