...
def query_Id_Id(id1, id2):
	...
	if json1.has_key('F') and json2.has_key('F'):
		jointFIdList = np.intersect1d(FIdList1, FIdList2)
	...
	if json1.has_key('AA') and json2.has_key('AA'):
		...
		jointAuIdList = np.intersect1d(AuIdList1, AuIdList2)
	...
	if json1.has_key('RId'):
		...
		jointRIdList = np.intersect1d(RIdList, Id2citedList)
	...
	if json1.has_key('RId'):
		...
		if json2.has_key('F'):
			for id1CitePaper in id1CitePapersInfo:
				if id1CitePaper.has_key('F'):
					...
					jointFIdList = np.intersect1d(FIdListTmp, FIdList2)
					for FId in jointFIdList:
		...
		if json2.has_key('AA'):
			for id1CitePaper in id1CitePapersInfo:
				if id1CitePaper.has_key('F'):
					...
					jointAuIdList = np.intersect1d(AuIdListTmp, AuIdList2)
					for AuId in jointAuIdList:
		...
		for id1CitePaper in id1CitePapersInfo:
			if id1CitePaper.has_key('RId'):
				...
				jointRIdList = np.intersect1d(RIdListTmp, Id2citedList)
				for RId in jointRIdList:
...
def query_Id_AuId(id1, auId2, json2):
	...
	if json1.has_key('RId'):
		...
		jointRIdList = np.intersect1d(paperIdList, RIdList)
		for RId in jointRIdList:
	...
	if json1.has_key('F'):
		...
		for paper in json2:
			if paper.has_key('F'):
				...
				jointFIdList = np.intersect1d(FIdListTmp, FIdList1)
				for FId in jointFIdList:
	...
	if json1.has_key('AA'):
		...
		for paper in json2:
			if paper.has_key('AA'):
				...
				jointAuIdList = np.intersect1d(AuIdListTmp, AuIdList1)
				for AuId in jointAuIdList:
	...
	if json1.has_key('RId'):
		for citePaperInfo in citePaperInfos:
			if citePaperInfo.has_key('RId'):
				...
				jointRIdList = np.intersect1d(RIdListTmp, paperIdList)
...
def main():
    ...
    # print len(query(2140190241, 1490955312))
    # print len(query(2126125555, 2153635508))
    # print len(query(2126125555, 2060367530))
    # print len(query(2140190241, 2121939561))
    # print len(query(2175015405, 1514498087))
    # print len(query(2251253715,2180737804))
